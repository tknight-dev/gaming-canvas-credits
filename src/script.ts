import {
	...
	GamingCanvasCreditsAuthor,
	GamingCanvasCreditsCategory,
...
class Transition {
	...
	private static gameLoop(): void {
		let cacheCanvasCursor: OffscreenCanvas,
			...
			canvas: HTMLCanvasElement = Transition.canvases[0],
			...
			inputs: GamingCanvasFIFOQueue<GamingCanvasInput> = Transition.inputs,
			pan: number,
			pointerArcTan: number,
			pointerX: number,
			pointerY: number,
		...
		// Initialize
		Transition.canvases[0].height = canvasHeight;
		Transition.canvases[0].width = canvasWidth;
		...
		const fire = (arcTan: number, timestampNow: number) => {
			...
			// Play Audio
			GamingCanvas.audioControlPlay(AssetId.FIRE);
			setTimeout(async () => {
				particle.audioInstance = await GamingCanvas.audioControlPlay(AssetId.TRAVEL, GamingCanvasAudioType.EFFECT, true);
			}, 40);
		};
		...
		const go = (timestampNow: number) => {
			...
			Transition.gameLoopRequest = requestAnimationFrame(go);
			...
			if (timestampDelta > fpms) {
				...
				context.lineTo(canvasWidthHalf + gameGunLength * Math.cos(pointerArcTan), canvasHeight - gameGunLength * Math.sin(pointerArcTan));
				context.stroke();
				...
				while (particle !== undefined) {
					if (particle.data.audioInstance !== null) {
						...
						GamingCanvas.audioControlVolume(particle.data.audioInstance, Math.max(0.05, 1 - volume / 300));
					}
			...
			if (timestampDelta !== 0) {
				...
				while (particle !== undefined) {
					...
					if (particle.data.x < 0 || particle.data.x > canvasWidth || particle.data.y < 0 || particle.data.y > canvasHeight) {
						...
						pan = particle.data.x / canvasWidthHalf - 1;
						volume = ((particle.data.x - canvasWidthHalf) ** 2 + (particle.data.y - canvasHeight) ** 2) ** 0.5;
						GamingCanvas.audioControlPlay(AssetId.EXPLOSION, GamingCanvasAudioType.EFFECT, false, pan, 0, Math.max(0.05, 2 - volume / 275));
					}
	...
	private static async initializeDOM(): Promise<void> {
		Transition.domControlsStart = <HTMLButtonElement>document.getElementById('controls-start');
		Transition.domControlsStart.onclick = async () => {
			console.log('start');
		};
	}

	private static async initializeGC(): Promise<void> {
		Transition.canvases = GamingCanvas.initialize(<HTMLElement>document.getElementById('video'), {
			aspectRatio: 16 / 9,
			audioEnable: true,
			canvasCount: 1,
			// debug: true,
		...
		Transition.inputs = GamingCanvas.getInputQueue();
		...
		// https://freesound.org/people/BMacZero/sounds/164102/
		assets.set(
			AssetId.TRAVEL,
	...
	public static async main(): Promise<void> {
		// Initialize
		Transition.initializeDOM();
		Transition.initializeGC();
